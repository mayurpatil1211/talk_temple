...
module.exports = {
    ...
    create(req, res) {
        if (req.body.email != '' && req.body.email != null && req.body.password != '' && req.body.password != null) {
                    ...
                    if (!user) {
                        ...
                        return User
                            ...
                            .catch(error => res.status(400).send({
                                message: "Not able to create User, Please check credentials",
                                message_code: 1001
                            }))
                    } else {
            ...
            return res.status(400).send({
                message: "Null shall\'t pass, check the credentials you are sending",
                message_code: 1003
            })
        }
            ...
            return User
                .findOne({
                    ...
                    if (user.password) {
                        ...
                        return res.status(400).send({
                            message: "May be User has to log in with social App",
                            message_code: 1006
                        })
                    }
    ...
    socialLogin(req, res) {
        if (req.body.email != '' && req.body.email != null && req.body.token != null) {
            return User.findOrCreate({
                ...
                if (!user) {
                    return res.status(404).send({
                        message: 'Could\'nt Log In Please Try again'
                    })
                } else {
                    var token = jwt.sign({
